...
<body>
	...
	<form id="modify_write"
		action="modify_writeAction.jsp?_id=<%=boardId%>" method="post">
		<div class="container">
			...
			<div>
				카테고리 <select name="category">
					<option value="read_notice"
						<%="read_notice".equals(vo.getCategory()) ? " selected" : ""%>>필독</option>
					<option value="cafe_notice"
						<%="cafe_notice".equals(vo.getCategory()) ? " selected" : ""%>>카페
						공지</option>
					<option value="free_board"
						<%="free_board".equals(vo.getCategory()) ? " selected" : ""%>>자유
						게시판</option>
					<option value="freshman_board"
						<%="freshman_board".equals(vo.getCategory()) ? " selected" : ""%>>새내기
						게시판</option>
					<option value="class_board"
						<%="class_board".equals(vo.getCategory()) ? " selected" : ""%>>강의평</option>
					<option value="foodstore_board"
						<%="foodstore_board".equals(vo.getCategory()) ? " selected" : ""%>>학교주변
						맛집</option>
				</select>
			</div>
			...
			<div class="wirte-btn">
				...
				<div class="wirte-upload">
					<a href="javascript:{}"
						onclick="document.getElementById('modify_write').submit();">등록</a>
				</div>
			</div>
		</div>
	</form>
